...
export interface EnrichedMarkdownTextInputInstance {
  ...
  insertCustomEmoji: (imageUri: string, size: number) => void;
  syncCustomEmojis: (entries: ReadonlyArray<{ offset: number; imageUri: string; size: number }>) => void;
...
export const EnrichedMarkdownTextInput = ({
  ...
  useImperativeHandle(ref, () => {
    ...
    return {
      ...
      insertCustomEmoji: (imageUri, size) =>
        Commands.insertCustomEmoji(commandRef, imageUri, size),
      syncCustomEmojis: (entries) =>
        Commands.syncCustomEmojis(commandRef, JSON.stringify(entries)),
      setPlainText: (text) => Commands.setPlainText(commandRef, text),
