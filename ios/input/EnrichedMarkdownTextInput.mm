...
@implementation EnrichedMarkdownTextInput {
  ...
  ENRMCustomEmojiStore *_customEmojiStore;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _customEmojiStore = [[ENRMCustomEmojiStore alloc] init];
    _lastTextLength = 0;
...
- (void)applyFormattingScopedToRange:(NSRange)scope
{
  ...
  [self reapplyCustomEmojiAttachmentsInRange:scope];
...
- (void)reapplyCustomEmojiAttachmentsInRange:(NSRange)scope
{
  NSTextStorage *textStorage = _textView.textStorage;
  NSString *plainText = textStorage.string;
  NSUInteger textLength = plainText.length;
  if (textLength == 0 || _customEmojiStore.allEntries.count == 0) {
    return;
  }
  ...
  for (ENRMCustomEmojiEntry *entry in [_customEmojiStore entriesIntersectingRange:scopeRange]) {
    if (entry.offset >= textLength) {
      continue;
    }
    if ([plainText characterAtIndex:entry.offset] != 0xFFFC) {
      [_customEmojiStore removeEntryAtOffset:entry.offset];
      continue;
    }
    ENRMCustomEmojiAttachment *attachment = [ENRMCustomEmojiAttachment attachmentWithImageUri:entry.imageUri
                                                                                         size:entry.size
                                                                                     baseFont:_formatterStyle.baseFont];
    NSDictionary *attributes = @{
      NSAttachmentAttributeName : attachment,
      NSFontAttributeName : _formatterStyle.baseFont,
    };
    [textStorage setAttributes:attributes range:NSMakeRange(entry.offset, 1)];
  }
...
{
  [_customEmojiStore adjustForEditAtLocation:editLocation
...
- (void)setPlainText:(NSString *)text
{
  ...
  [_editSession enterPhase:ENRMEditPhaseFormatting];
  [self resetBaseTypingAttributes];
  ENRMSetPlainText(_textView, plain);
  [_formattingStore clearAll];
  [_blockStore clearAll];
  [_customEmojiStore clearAll];
  _lastTextLength = plain.length;
  _lastSelectedRange = NSMakeRange(plain.length, 0);
  _textView.selectedRange = _lastSelectedRange;
  [_editSession exitPhase];
  [self applyFormatting];
  [self updatePlaceholderVisibility];
  if (plain.length == 0) {
...
- (void)insertCustomEmoji:(NSString *)imageUri size:(CGFloat)size
{
  if (imageUri.length == 0 || size <= 0) {
    return;
  }
  ...
  [self replaceTextInRange:selection withText:replacement formattingRanges:@[]];
  [_customEmojiStore addEntry:[ENRMCustomEmojiEntry entryWithOffset:editLocation imageUri:imageUri size:size]];
  [self reapplyCustomEmojiAttachmentsInRange:NSMakeRange(editLocation, 1)];
  _textView.selectedRange = NSMakeRange(editLocation + 1, 0);
  [_inputEventEmitter emitOnChangeSelection];
  [self requestHeightUpdate];
  [self scheduleRelayoutIfNeeded];
}

- (void)syncCustomEmojis:(NSString *)json
{
  ...
  NSMutableArray<ENRMCustomEmojiEntry *> *entries = [NSMutableArray array];
  for (id item in (NSArray *)parsed) {
    if (![item isKindOfClass:[NSDictionary class]]) {
      continue;
    }
    NSDictionary *dict = (NSDictionary *)item;
    id offsetValue = dict[@"offset"];
    id uriValue = dict[@"imageUri"];
    id sizeValue = dict[@"size"];
    if (![offsetValue isKindOfClass:[NSNumber class]] || ![uriValue isKindOfClass:[NSString class]] ||
        ![sizeValue isKindOfClass:[NSNumber class]]) {
      continue;
    }
    [entries addObject:[ENRMCustomEmojiEntry entryWithOffset:[offsetValue unsignedIntegerValue]
                                                    imageUri:(NSString *)uriValue
                                                        size:[sizeValue doubleValue]]];
  }

  [_customEmojiStore setEntries:entries];
  [self reapplyCustomEmojiAttachmentsInRange:NSMakeRange(0, ENRMGetPlainText(_textView).length)];
  [self requestHeightUpdate];
  [self scheduleRelayoutIfNeeded];
}
